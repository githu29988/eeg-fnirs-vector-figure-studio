...
function ConfusionMatrixChart() {
  ...
  const expertSchema: ExpertSchema = [
    ...
    },
    {
      label: '分类器',
      fields: [
        { type: 'number', key: 'sep', label: '均值分离度 μ', min: 0, max: 6, step: 0.01, value: separation, onChange: setSeparation, slider: true, format: (v) => v.toFixed(2) },
      ],
    },
    {
      label: '显示',
  ...
  const inspirations: InspirationPreset[] = [
    {
      ...
      label: '强分类器',
      hint: '基线',
      description: '高分离度 + 大样本 + Viridis 配色。',
      apply: () => {
        setN(2000);
        setSeparation(2.6);
        setNormalize(true);
    ...
    },
    {
      ...
      label: '小样本预试',
      hint: '警示',
      description: 'n=160 原始计数，凸显样本量噪声。',
      apply: () => {
        setN(160);
        setSeparation(1.6);
        setNormalize(false);
    ...
    },
    {
      ...
      label: 'Magma 配色',
      hint: '色彩',
      description: '同一模型，换为热图风格的 magma 配色。',
      apply: () => {
        setColormap('magma');
      },
    },
  ...
  return (
    <ChartShell
      ...
      notes={
        <p>
          合成 4 状态癫痫分期分类器的多类混淆矩阵。每个单元格表示
          <code>按行归一化的召回率</code>或原始计数（由开关切换）。
          单元值以纯文本渲染，便于辅助阅读与黑白打印。
        </p>
      }
      figure={
        <FigureFrame
...
registerChart({
  ...
  category: 'evaluation',
  summary: '多类混淆矩阵，提供按行归一化的召回率视图、感知均匀配色与可读单元标签。',
