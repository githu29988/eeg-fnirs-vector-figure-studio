    ...
    <section className="rounded-lg border border-ink-700 bg-ink-900 p-4">
      <header className="mb-3">
        ...
        <p className="text-[11px] text-ink-300">
          拖入 EDF / BIDS 文件包以实际记录驱动支持的图表。未加载时使用合成数据。
        </p>
      </header>
      ...
      >
        <p className="text-center text-ink-300">
          ...
          <code>_electrodes.tsv</code> / <code>_eeg.json</code>
        </p>
      ...
      <div className="mt-3 text-[11px]">
        {status.kind === 'idle' ? (
          <p className="text-ink-300">状态：合成数据（未加载文件）。</p>
        ) : null}
        {status.kind === 'loading' ? (
        ...
        {status.kind === 'loaded' ? (
          <div className="space-y-1 text-ink-100">
            <p>
              已加载 <span className="font-mono">{status.dataset.variant}</span> ·{' '}
